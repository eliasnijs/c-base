 ...
 * |
 * | uint32 *ptr = (uint32 *)arena_alloc(&arena, sizeof(uint32));
 * | *ptr = 42;
 * |
 * | arena_reset(&arena);
 * |
 * | uint32 *ptr2 = (uint32 *)arena_alloc(&arena, sizeof(uint32));
 * | *ptr2 = 42;
 * |
 * | // ptr == ptr2
 *
...
struct Arena {
	...
	uint32 len;           /* length of the reserved memory */
	uint32 offset_curr;   /* current offset into the bufer */
	uint32 offset_prev;   /* previous offset into the bufer */
};
...
/* initialise an arena */
internal void
arena_init(Arena *a, void *backbuffer, uint32 l) {
...
internal void *
arena_alloc_align(Arena *a, uint32 len, uint32 align) {
...
/* IMPORTANT(Elias): memory is not guaranteed to be 0! */
internal void *
arena_alloc(Arena *a, uint32 len) {
